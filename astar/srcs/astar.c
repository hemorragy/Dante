/*
** depth.c for depth in /home/antoin_g/rendu/C_prog_elem/dante/profondeur
**
** Made by Loïs
** Login   <antoin_g@lois-HP-EliteBook-840-G2>
**
** Started on  Thu Apr 28 03:29:17 2016 Loïs
** Last update Thu May  5 20:06:36 2016 Loïs
*/

#include "functions.h"
#include "defines.h"

static void	add_one(int x, int y, t_utils *utils)
{
  t_chain	*new;

  new = xmalloc(sizeof(t_chain));
  new->next = NULL;
  new->x = x;
  new->y = y;
  if (utils->first == NULL)
    utils->first = new;
  else
    utils->last->next = new;
  utils->last = new;
}

static void	free_chain(t_chain *first)
{
  t_chain	*tmp;

  while (first)
    {
      tmp = first;
      first = first->next;
      free(tmp);
    }
}

void	check_all_sens(int **tab, const t_map *map,
		       const t_chain *tmp, t_prio *prio)
{
  if (tmp->x > 0 && tab[tmp->y][tmp->x - 1] == UNDEF)
    {
      tab[tmp->y][tmp->x - 1] = tab[tmp->y][tmp->x] + 1;
      add_one(tmp->x - 1, tmp->y, &prio->prio);
    }
  if (tmp->x < map->width - 1 && tab[tmp->y][tmp->x + 1] == UNDEF)
    {
      tab[tmp->y][tmp->x + 1] = tab[tmp->y][tmp->x] + 1;
      add_one(tmp->x + 1, tmp->y, &prio->back);
    }
  if (tmp->y > 0 && tab[tmp->y - 1][tmp->x] == UNDEF)
    {
      tab[tmp->y - 1][tmp->x] = tab[tmp->y][tmp->x] + 1;
      add_one(tmp->x, tmp->y - 1, &prio->prio);
    }
  if (tmp->y < map->height - 1 && tab[tmp->y + 1][tmp->x] == UNDEF)
    {
      tab[tmp->y + 1][tmp->x] = tab[tmp->y][tmp->x] + 1;
      add_one(tmp->x, tmp->y + 1, &prio->back);
    }
}

static void	resolve_it(t_prio *prio, int **tab, const t_map *map)
{
  int		check;
  t_chain	*tmp;
  t_chain	*save;

  check = 0;
  while (!check)
    {
      if (prio->prio.first)
	{
	  tmp = prio->prio.first;
	  save = tmp;
	  prio->prio.first = NULL;
	  while (tmp && !check)
	    {
	      check = check_next(tab, map, tmp, prio);
	      tmp = tmp->next;
	    }
	  free_chain(save);
	}
      else
	check = go_back(prio, check, tab, map);
      check = (!prio->back.first && !prio->prio.first) ? 1 : 0;
    }
  free_chain(prio->back.first);
  free_chain(prio->prio.first);
}

void		go_deeper(int x, int y, int **tab, const t_map *map)
{
  t_prio	prio;

  prio.prio.first = NULL;
  prio.back.first = NULL;
  if (map->first->str[0] == 'X' ||
      map->last->str[my_strlen(map->last->str) - 1] == 'X')
    return ;
  add_one(x, y, &prio.prio);
  resolve_it(&prio, tab, map);
}
